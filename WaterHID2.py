...
def iterate_w(l_pipes, equation, co_m, a_21, a_12, a_10, n_d, q_o, h_o, q_d, ide, error, i_max):
    cont = 0
    while cont < i_max:
        l_a11 = []
        for i in l_pipes:
            l_a11.append(i.alpha(equation) * i.flow ** (co_m - 1) + i.beta + i.lamb / i.flow)
        a_11 = np.matrix(diagonal(l_a11).tolist)
        h_nxt = -(a_21 * ((n_d * a_11) **-1* a_12)) **-1* (a_21 * (n_d**-1* (q_o + a_11**-1* a_10 * h_o)) + q_d - a_21 * q_o)
        q_nxt = ide * q_o - n_d // q_o - (n_d * a_11) **-1*  (a_12 * h_nxt + a_10 * h_o)
        pres = np.linalg.norm(abs(q_o) - abs(q_nxt))
        if pres < error:
            break
        q_o = abs(q_nxt)
...
def water_net(l_pipes, l_nodes, l_reservoir, eq='D-W', error=1e-3, i_max=10):
    """
    EXAMPLE:
    node1 = Node()
    ...
    ide = np.matrix(di['ide'].tolist)
    a_21 = np.matrix(a_21.tolist)
    n_d = np.matrix(n_d.tolist)
    a_12 = np.matrix(a_12.tolist)
    a_10 = np.matrix(a_10.tolist)
    h_o = np.matrix(h_o.tolist)
    q_d = np.matrix(q_d.tolist)
    q_o = np.matrix(q_o.tolist)
    cond, res = iterate_w(l_pipes, equation, co_m, a_21, a_12, a_10, n_d, q_o, h_o, q_d, ide, error, i_max)
    if cond:
        q_nxt = res['q']
        h_nxt = res['h']
        for i, j in enumerate(l_pipes):
...
def weight(l_d, l_pipes, l_nodes, l_reservoir, h_v, diam):
    ...
    for i, j in enumerate(l_pipes2):
        j.diam = l_d[i]
    cond, _ = water_net(l_pipes2, l_nodes, l_reservoir)
    if cond:
        f_p = 0
        cost = 0
        for i in l_nodes:
...
def designer_e_g(l_pipes, l_nodes, l_res, diam, cond, ge=10, n_p=10, bar=None):
...
l_r = [res1]
pipe1 = Pipe()
pipe1.n_i, pipe1.n_f, pipe1.flow, pipe1.diam, pipe1.long, pipe1.vis, pipe1.ks = res1, node1,.1, .254, 500, 1.14E-6, 6E-5
pipe2 = Pipe()
pipe2.n_i, pipe2.n_f, pipe2.flow, pipe2.diam, pipe2.long, pipe2.vis, pipe2.ks, pipe2.kl = node1, node2, .1, .1524, 400, 1.14E-6, 6E-5, 10
pipe3 = Pipe()
pipe3.n_i, pipe3.n_f, pipe3.flow, pipe3.diam, pipe3.long, pipe3.vis, pipe3.ks = node3, node2, .1, .1016, 200, 1.14E-6, 6E-5
pipe4 = Pipe()
pipe4.n_i, pipe4.n_f, pipe4.flow, pipe4.diam, pipe4.long, pipe4.vis, pipe4.ks = node4, node3, .1, .1524, 400, 1.14E-6, 6E-5
pipe5 = Pipe()
pipe5.n_i, pipe5.n_f, pipe5.flow, pipe5.diam, pipe5.long, pipe5.vis, pipe5.ks = node1, node4, .1, .1016, 200, 1.14E-6, 6E-5
pipe6 = Pipe()
pipe6.n_i, pipe6.n_f, pipe6.flow, pipe6.diam, pipe6.long, pipe6.vis, pipe6.ks = node5, node4, .1, .2032, 600, 1.14E-6, 6E-5
pipe7 = Pipe()
pipe7.n_i, pipe7.n_f, pipe7.flow, pipe7.diam, pipe7.long, pipe7.vis, pipe7.ks = res1, node5, .1, .254, 300, 1.14E-6, 6E-5
l_p = [pipe1, pipe2, pipe3, pipe4, pipe5, pipe6, pipe7]
water_net(l_p, l_n, l_r, i_max=45)
